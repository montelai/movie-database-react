...
export default function SingleMovie(props) {
    ...
    let movieData = props.data.data.filter(movie => movie.id != props.movieId)[0];
    return (
        <Paper style={{'backgroundColor':'#1e1e1e'}}>
            <Grid container  spacing={3}>
                <Grid item style={{'padding':'10px', 'justifyContent':'center', 'display':'flex','flex':'2'}}><img src={`https://image.tmdb.org/t/p/w500/${movieData.poster_path}`} style={{'margin':'10px', 'height':'50%'}}/></Grid>
